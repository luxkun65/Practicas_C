/*
Ejemplo que declara una matriz de tipo int (enteros) de tamaño variable.
...
void limpiar(){
    for(int f = 0; f < cant_filas; f++){
        for(int c = 0; c < cant_columnas; c++){
            enteros[f][c] = 0;//inicializar cada posición de la matriz en cero
        }
    }
}
...
void leer_tamano(){
    ...
    enteros = new int *[cant_filas];//crear la cantidad filas
    for(int f = 0; f < cant_filas; f++) {
       enteros[f] = new int[cant_columnas];//crear la cantidad columnas
    }
