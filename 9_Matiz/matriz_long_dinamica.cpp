/*
Ejemplo que declara una matriz de tipo long (enteros) de tamaño variable.
...
void limpiar(){
    for(int f = 0; f < cant_filas; f++){
        for(int c = 0; c < cant_columnas; c++){
            largos[f][c] = 0;//inicializar cada posición de la matriz en cero
        }
    }
}
...
void leer_tamano(){
    ...
    largos = new long *[cant_filas];//crear la cantidad filas
    for(int f = 0; f < cant_filas; f++) {
       largos[f] = new long[cant_columnas];//crear la cantidad columnas
    }
